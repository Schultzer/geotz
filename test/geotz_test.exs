defmodule GeotzTest do
  ...
  describe "lookup/2" do
    for {lat, lat, expected} <- @int_waters do
      test "international waters TZ: " <> expected  do
        assert expected == Geotz.lookup(lat, lat)
      end
    end

    for {lat, lat, expected} <- @hand_crafted do
      test "handcrafted TZ: " <> expected  do
        assert expected == Geotz.lookup(lat, lat)
      end
    end

    for {lat, lat, expected} <- @auto_generated do
      test "auto generated TZ: " <> expected  do
        assert expected == Geotz.lookup(lat, lat)
      end
    end
  end
